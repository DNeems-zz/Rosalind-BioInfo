...
AA-AA AA-Aa AA-aa Aa-Aa Aa-aa aa-aa Return: The expected number of offspring
displaying the dominant phenotype in the next generation, under the assumption
that every couple has exactly two offspring.

Sample Dataset

1 0 0 1 0 1 Sample Output
...
def main():
  Input= (open(sys.argv[1]).readline().strip().split(' '))
  Freq=list()
  [Freq.append(int(f)) for f in Input]
  GT=[('AA','AA'),('AA','Aa'),('AA','aa'),('Aa','Aa'),('Aa','aa'),('aa','aa')]
  Expected_Domintant=0
  for GT_pair,Num in zip(GT,Freq):
    genotype_Freq=Cross(GT_pair[0],GT_pair[1])[1]
    for F_pair in genotype_Freq:
      if F_pair[0].count('A') >0:
        Expected_Domintant+=(F_pair[1]/4.0)*Num
  with open('Ex22_Output.txt', 'a') as f:
    f.write('Genotype: Frequency\n')
    for GT_pair,Num in zip(GT,Freq):
